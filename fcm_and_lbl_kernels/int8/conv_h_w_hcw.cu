...
#if (FUSION_MODE == ALL_MODES || FUSION_MODE == NOT_FUSED) && DATA_LAYOUT == HCW
                               ...
                               const scales_dt dw_relu_threshold)
{
    ...
    {
        const int abs_row_index = (block_h * TILE_H_H_W + thread_h) * strides - padding_top;

        if ((block_h * TILE_H_H_W + thread_h) < dw_ofm_height)
        {

            // for (int i_w = 0; i_w < tile_w; i_w++)
            {
                pss_dt sum0 = 0, sum1 = 0, sum2 = 0, sum3 = 0;
                const int abs_w_write = block_w * TILE_W_H_W + thread_w;
                const int abs_w_read = (block_w * TILE_W_H_W + thread_w) * strides - padding_left;

                const int base_index_in_ofms = (block_h * TILE_H_H_W + thread_h) *
                                                   dw_ofm_width *
                                                   compact_layer_num_filters_and_depth +
                                               abs_dw_f_compact * dw_ofm_width;
                                               
                const int base_index_in_ifms = abs_row_index * dw_ifms_width_depth +
                                               abs_dw_f_compact * dw_ifm_width;
                            ...
                            const int layer_activation)
{
    ...
    const int thread_hw = thread_h * TILE_H_H_W + thread_w;
    ...
    // for (int o_f = 0; o_f < packed_ofms_per_thread_f; o_f++)
    {
        ...
        // for (int h = 0; h < rows_per_thread; h++)
        {
            const int row_offet_in_tile = thread_h * compact_layer_depth * TILE_W_H_W;
            const int base_index_in_ofms = (block_h * TILE_H_H_W + thread_h) * pw_ofm_width * compact_layer_num_filters +
                                           abs_f_compact * pw_ofm_width;

            // for (int i_w = 0; i_w < tile_w; i_w++)
            if (block_h * TILE_H_H_W + thread_h < pw_ofm_width)
            {
                const int abs_w_write = block_w * TILE_W_H_W + thread_w;
                if (abs_w_write < pw_ofm_width)
                {
                    ...
                    for (int d = 0; d < compact_layer_depth; d++)
                    {
                        int base_index_in_ifms = (block_h * TILE_H_H_W + thread_h) * pw_ifm_width * compact_layer_depth +
                                                 d * pw_ifm_width;
                        ...
                        float &exec_time)
{
    ...
    if (test_iteration == 0)
    {
        printf("(conv_h_w): layer %d\n", l_specs.layer_index);
    }

    const int padded_tile_w = least_pow_of_2_geq(l_specs.layer_ifm_width +
                                                 l_specs.padding_left + l_specs.padding_right);

    dim3 threads(TILE_W_H_W, TILE_H_H_W, 1);
    dim3 blocks((ofms_width + TILE_W_H_W) / TILE_W_H_W, (ofms_height + TILE_H_H_W) / TILE_H_H_W, compact_layer_num_filters);
    if (l_specs.conv_layer_type == DW_CONV)
    {
        ...
        dw_conv3x3_h_w<<<blocks, threads>>>(ifms, ofms, dw_weights, fused_scales, fused_zps,
                                            l_specs.layer_depth,
    ...
    else if (l_specs.conv_layer_type == PW_CONV)
    {
        ...
        pw_conv_h_w<<<blocks, threads>>>(ifms, ofms, pw_weights, fused_scales, fused_zps,
