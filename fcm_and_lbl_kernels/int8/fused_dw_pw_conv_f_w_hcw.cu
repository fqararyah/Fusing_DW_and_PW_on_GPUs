...
#if COMPILE_FUSED && (FUSION_MODE == ALL_MODES || FUSION_MODE == FUSED_F_W) && DATA_LAYOUT == HCW
                                       ...
                                       const int packed_ofms_per_thread_f)
{
    ...
    if (abs_w_write < dw_ofm_width)
    {
        ...
        weight_val0 = __shfl_sync(__activemask(), my_weight, 0);
        weight_val1 = __shfl_sync(__activemask(), my_weight, 1);
        weight_val2 = __shfl_sync(__activemask(), my_weight, 2);
        ...
        weight_val0 = __shfl_sync(__activemask(), my_weight, F_W_PARALLEL_W);
        weight_val1 = __shfl_sync(__activemask(), my_weight, F_W_PARALLEL_W + 1);
        weight_val2 = __shfl_sync(__activemask(), my_weight, F_W_PARALLEL_W + 2);
        ...
        weight_val0 = __shfl_sync(__activemask(), my_weight, 2 * F_W_PARALLEL_W);
        weight_val1 = __shfl_sync(__activemask(), my_weight, 2 * F_W_PARALLEL_W + 1);
        weight_val2 = __shfl_sync(__activemask(), my_weight, 2 * F_W_PARALLEL_W + 2);
    ...
    for (int o_f = 0; o_f < packed_ofms_per_thread_f; o_f++)
    {
        if (thread_f < pw_compact_layer_num_filters)
        {
            ...
            for (int h = 0; h < rows_per_thread; h++)
            {
                ...
                const int base_index_in_ofms = abs_row_index_in_ofms * pw_ofm_width * pw_compact_layer_num_filters +
...
void fused_dwpw_convolutionGPU(fms_dt *ifms, fms_dt *ofms,
