            ...
            // for (int i_w = 0; i_w < tile_w; i_w++)
            {
                ...
                if (abs_w_write < dw_ofm_width)
                {
                    for (int c_h = 0; c_h < FILTER_3x3_DIM; c_h++)
                    {
                        ...
                        // if ((block_h * parallel_h + thread_h) == 14 && abs_w_write == 29 && abs_dw_f_compact == 3)
                        // {
                    ...
                    //     sum0 += fused_zp0;
                    //     printf("\n gpu: %d >> %d\n", sum0, q0);
                    //     float scaled_pss = sum0 * scale0;
                    //     printf("%f\n", scale0);
                    //     printf("%f\n", scaled_pss);
                    //     scaled_pss += dw_ofms_zp;
                    //     printf("%f\n", scaled_pss);
            ...
            if (block_h * parallel_h + thread_h < pw_ofm_width)
            {
                ...
                if (abs_w_write < pw_ofm_width)
                {
                    ...
                    for (int d = 0; d < compact_layer_depth; d++)
                    {
                        ...
                        // if (block_h * parallel_h + thread_h == 0 && abs_w_write == 0 && abs_f_compact == 0)
                        // {
                        //     printf("%d * %d\n ", EXTRACT_8_32(pw_weights[base_index_pw_weights + d], 0), EXTRACT_8_32(ifms_val, 0));
                        //     printf("%d * %d\n", EXTRACT_8_32(pw_weights[base_index_pw_weights + d], 1), EXTRACT_8_32(ifms_val, 1));
                        //     printf("%d * %d\n", EXTRACT_8_32(pw_weights[base_index_pw_weights + d], 2), EXTRACT_8_32(ifms_val, 2));
                        //     printf("%d * %d\n", EXTRACT_8_32(pw_weights[base_index_pw_weights + d], 3), EXTRACT_8_32(ifms_val, 3));
                        // }
                    }
                    ...
                    // if (block_h * parallel_h + thread_h == 0 && abs_w_write == 0 && abs_f_compact == 0)
                    // {
                    //     printf("\n gpu: %d >> %d\n", sum0, quant_relu6(sum0, scale0, fused_zp0, pw_ofms_zp, pw_relu_threshold));
                    // }
                            ...
                            float &exec_time)
{
    ...
    if (test_iteration == 0)
    {
        printf("(conv_h_w_chw_int8): layer %d\n", l_specs.layer_index);
    }
