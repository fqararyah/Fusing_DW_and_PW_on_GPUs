...
#if (FUSION_MODE == ALL_MODES || FUSION_MODE == FUSED_F_W) && DATA_LAYOUT == CHW && DATA_TYPE == INT8_DTYPE
                               ...
                               const scales_dt dw_relu_threshold)
{
    ...
    for (int h = 0; h < rows_per_thread; h++)
    {
        ...
        const int base_index_in_ofms = (block_h * rows_produced_each_time + (thread_h * rows_per_thread + h)) *
                                           dw_ofm_width +
                                       thread_f * dw_ofm_height * dw_ofm_width;
            ...
            // for (int i_w = 0; i_w < tile_w; i_w++)
            {
                ...
                if (abs_w_write < dw_ofm_width)
                {
                    for (int c_h = 0; c_h < FILTER_3x3_DIM; c_h++)
                    {
                        ...
                        // if (abs_row_index == 0 && abs_w_write == 0 && thread_f == 0)
                        // {
                        //     printf("%d % d %d\n", EXTRACT_8_32(ifms_val0, 0), EXTRACT_8_32(ifms_val1, 0), EXTRACT_8_32(ifms_val2, 0));
                        //     printf("%d % d %d\n", EXTRACT_8_32(weight_val0, 0), EXTRACT_8_32(weight_val0, 1), EXTRACT_8_32(weight_val0, 2));
                        // }
                    }
    ...
    for (int o_f = 0; o_f < packed_ofms_per_thread_f; o_f++)
    {
        if (thread_f < compact_layer_num_filters)
        {
            ...
            for (int h = 0; h < rows_per_thread; h++)
            {
                ...
                const int base_index_in_ofms = (block_h * F_W_TILE_H + (thread_h * rows_per_thread + h)) * pw_ofm_width +
                                               abs_f_compact * pw_ofm_width * pw_ofm_width; // TODO
                ...
                if (block_h * F_W_TILE_H + (thread_h * rows_per_thread + h) < pw_ofm_width)
                {
                    ...
                    if (abs_w_write < pw_ofm_width)
                    {
                        ...
                        {
                            int base_index_in_ifms = (block_h * F_W_TILE_H + (thread_h * rows_per_thread + h)) * pw_ifm_width +
                                                     d * pw_ifm_width * pw_ifm_width; // TODO
                        ...
                        float &exec_time)
{
    ...
    if (test_iteration == 0)
    {
        printf("(conv_f_w_chw_int8): : layer %d\n", l_specs.layer_index);
    }
