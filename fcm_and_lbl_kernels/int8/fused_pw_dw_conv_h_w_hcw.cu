...
#if COMPILE_FUSED && (FUSION_MODE == ALL_MODES || FUSION_MODE == FUSED_H_W) && DATA_LAYOUT == HCW
    ...
    if (abs_h < pw_ofm_height)
    {
        ...
        if (abs_w_write < pw_ofm_width)
        {
            ...
            for (int d = 0; d < compact_layer_depth; d++)
            {
                ...
                fms_dt ifms_val = ifms[base_index_in_ifms + abs_w_write];
            ...
            // for (int h = 0; h < rows_per_thread; h++)
            {
                ...
                if (thread_h < parallel_h / strides)
                {
                    ...
                    if (thread_w < parallel_w / strides)
                    {
                        ...
                        ofms[base_index_in_ofms + abs_w_write] = PACK_32_8s(quant_relu6(sum0, scale0, fused_zp0, dw_ofms_zp, dw_relu_threshold),
