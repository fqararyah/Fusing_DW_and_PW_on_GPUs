...
#if COMPILE_FUSED && (FUSION_MODE == ALL_MODES || FUSION_MODE == FUSED_H_W) && DATA_LAYOUT == HWC
...
void inline __device__ padd_left_right(fms_dt *ifms_tile,
                                       const int padding_left,
                                       const int padding_right,
                                       const int dw_ifms_width,
                                       ...
                                       const fms_dt packed_ifm_zp)
{
    ...
    {
        ifms_tile[thread_w * FW_MAX_PADDED_TILE_W + w] = packed_ifm_zp;
    }
    for (int w = 0; w < padding_right; w++)
    {
        ifms_tile[thread_w * FW_MAX_PADDED_TILE_W + w + dw_ifms_width + padding_left] = packed_ifm_zp;
    }
}

inline __device__ void load_tile_fw(fms_dt *ifms, fms_dt ifms_tile[FILTER_3x3_DIM * FW_MAX_PADDED_TILE_W],
                                    const int compact_layer_depth,
                                    ...
                                    const int tile_in_h,
                                    const int thread_w,
                                    const int rows_consumed_each_time,
                                    const fms_dt packed_ifm_zp)
{

    const int num_iters = FILTER_3x3_DIM;
    const int first_time_rows = padding_top;

    for (int h = 0; h < num_iters; h++)
    {
        // padding top and bottom
        if ((tile_in_h * rows_consumed_each_time + h - first_time_rows) < 0 ||
            (tile_in_h * rows_consumed_each_time + h - first_time_rows) >= ifm_height)
        {
            ifms_tile[h * FW_MAX_PADDED_TILE_W + padding_left + thread_w] = packed_ifm_zp;
        }
        else
        {
            const int base_index =
                ((tile_in_h * rows_consumed_each_time + h - first_time_rows) * compact_layer_depth + starting_d) * ifm_width;

            ifms_tile[h * FW_MAX_PADDED_TILE_W + padding_left + thread_w] =
                ifms[base_index + thread_w];
        }
        // padding left
        if (thread_w < padding_left)
        {
            ifms_tile[h * FW_MAX_PADDED_TILE_W + thread_w] = packed_ifm_zp;
        }
        // padding right
        if (thread_w < padding_right)
        {
            ifms_tile[h * FW_MAX_PADDED_TILE_W + ifm_width + padding_left + thread_w] = packed_ifm_zp;
        }
    }
}
                                  ...
                                  const int compact_layer_depth_to_parallel_hw_ratio)
{
    ...
    __shared__ weights_dt dw_filter_weights[FILTER_3x3_DIM * PACKED_ITEMS];
    ...
    if (abs_h < pw_ofm_height)
    {
        ...
        if (abs_w_write < pw_ofm_width)
        {
            ...
            if (thread_h == 0 && thread_w == 0)
            {
                for (int c_h = 0; c_h < FILTER_3x3_DIM; c_h++)
                {
                    weights_dt weight_val0 =
                        dw_weights[dw_layer_weights_offset + (c_h * FILTER_3x3_DIM) * compact_layer_num_filters_and_depth + abs_dw_f_compact];
                    weights_dt weight_val1 =
                        dw_weights[dw_layer_weights_offset + (c_h * FILTER_3x3_DIM + 1) * compact_layer_num_filters_and_depth + abs_dw_f_compact];
                    weights_dt weight_val2 =
                        dw_weights[dw_layer_weights_offset + (c_h * FILTER_3x3_DIM + 2) * compact_layer_num_filters_and_depth + abs_dw_f_compact];
                    for (int f = 0; f < PACKED_ITEMS; f++)
                    {
            ...
            int base_index_in_ifms = abs_h * pw_ifm_width * compact_layer_depth +
                                     abs_w_write * compact_layer_depth;
            for (int d = 0; d < compact_layer_depth; d++)
            {

                const int d_offset = d * PACKED_ITEMS;

                fms_dt ifms_val = ifms[base_index_in_ifms + d];
                ...
                // if (base_index_in_ifms == 0 && abs_dw_f_compact == 0)
                // {
                //     printf("%d, %d, %d, %d, %d >>>>>>>>>>>>>>>>>\n",base_index_in_ifms + d, EXTRACT_8_32(ifms_val, 0),
                //            EXTRACT_8_32(ifms_val, 1),
                //            EXTRACT_8_32(ifms_val, 2),
                //            EXTRACT_8_32(ifms_val, 3));
                // }
            }
            ...
            // for (int h = 0; h < rows_per_thread; h++)
            {
                ...
                const int base_index_in_ofms = (block_h * parallel_h / strides + thread_h) *
                                                   dw_ofm_width * compact_layer_num_filters_and_depth +
                                               abs_w_write * compact_layer_num_filters_and_depth + abs_dw_f_compact;
...
void fused_pw_dw_convolutionGPU_h_w(fms_dt *ifms, fms_dt *ofms,
