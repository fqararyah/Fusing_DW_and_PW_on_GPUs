...
__global__ void pw_conv_h_w_chw(fms_dt *ifms, fms_dt *ofms, weights_dt *pw_weights,
                            ...
                            const int parallel_hw)
{
    ...
    __shared__ weights_dt weights_tile[TILE_F_H_W][256]; // TODO
...
__global__ void dw_conv3x3_h_w_chw(fms_dt *ifms, fms_dt *ofms, weights_dt *weights,
                               fused_scales_dt *fused_scales,
                               biases_dt *fused_zps,
                        ...
                        float &exec_time)
{
    ...
    if (l_specs.conv_layer_type == DW_CONV)
    {
        ...
        dw_conv3x3_h_w_chw<<<blocks, threads>>>(ifms, ofms, dw_weights, fused_scales, fused_zps,
                                            l_specs.layer_depth,
    ...
    else if (l_specs.conv_layer_type == PW_CONV)
    {
        ...
        dim3 threads(parallel_w, parallel_h, 1);
        dim3 blocks((ofms_width + parallel_w) / parallel_w, (ofms_height + parallel_h) / parallel_h,
                    compact_layer_num_filters / TILE_F_H_W);

        pw_conv_h_w_chw<<<blocks, threads>>>(ifms, ofms, pw_weights, fused_scales, fused_zps,
